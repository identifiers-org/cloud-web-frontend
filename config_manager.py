...
def is_show_logs_on_console():
    global __show_logs_on_console
    return __show_logs_on_console
...
class AppConfigManager(ConfigurationManager):
    ...
    def __init__(self, configuration_object, configuration_file):
        ...
        if is_show_logs_on_console():
            ...
            lformatter = logging.Formatter(_logger_formatters[_console_log_level])
            lhandler.setFormatter(lformatter)
            self.__log_handlers.append(lhandler)
            # Add the handlers to my own logger
            self._logger.addHandler(lhandler)
    ...
    def get_path_chrome_driver(self):
        # TODO - Selenium path could be externalized later down the road, if it makes sense
        path_selenium_binaries = os.path.join(self.get_folder_bin(), 'selenium')
        if platform.system() == 'Darwin':
            return os.path.join(path_selenium_binaries, "chromedriver-mac")
        elif platform.system() == 'Linux':
            return os.path.join(path_selenium_binaries, "chromedriver-linux")
        else:
            return os.path.join(path_selenium_binaries, "chromedriver.exe")
